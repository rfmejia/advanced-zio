      ...
      test("apply") {
        ...
        test("fromZIO") {
          ...
          for {
            ...
            stream = ZStream.fromEffect(effect)
            _      <- stream.foreach(value => ref.update(_ + value))
            v      <- ref.get
        ...
        test("fromFile") {
          ...
          for {
            ...
            _     <- ZStream.fromFile(path).transduce(decode).foreach(Console.printLine(_))
      ...
      test("runHead") {
        ...
        test("foldZIO") {
          ...
          for {
            ...
            //           map + (question -> answer)
            //       }
            map <- questions.foldZIO(Map.empty[String, String]) {
                    case (map, question) =>
                      for {
...
object AdvancedConstructors extends DefaultRunnableSpec {
  def spec =
    suite("AdvancedConstructors") {
       ...
       */
      test("unfold") {
        val fibs: ZStream[Any, Nothing, Int] = ZStream(0, 1) ++
          ZStream.unfold((0, 1)) {
            case (n, m) =>
              val next = n + m
              Some((next, (m, next)))
